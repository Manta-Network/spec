...
\Asset{s} are the basic building-blocks of \emph{transactions} which consume as set of input \Asset{s} and produce as set of transformed output \Asset{s}. To preserve the economic value stored in \Asset{s}, the sum of the input \AssetValue{s} must balance the sum of the output \AssetValue{s}, and all assets in a single transaction must have the same \AssetId{}\footnotemark{}. This is called a \emph{balanced transfer}: no \AssetValue{} is created or destroyed in the process. The \MantaPay{} protocol uses a distributed algorithm called \Transfer{} to perform balanced transfers and ensure that they are valid.
...
In order for \MantaPay{} participants to receive \Asset{s} via the \Transfer{} protocol, they create an \emph{address} which they use as a unique identifier to represent them on the ledger.

\begin{center}
    \vspace{1em}
    \begin{mdframed}[leftmargin=0.25\textwidth, rightmargin=0.25\textwidth]
        \begin{center}
    \begin{tikzcd}
        & \sk \arrow[r, Rightarrow] \arrow[ld, bend right] & \pk \arrow[rd, bend left] & \\
        {(\sk, \svk)} & & & {(\pk, \pvk)} \\
        & \svk \arrow[r, Rightarrow] \arrow[lu, bend left] & \pvk \arrow[ru, bend right] &
    \end{tikzcd}
        \end{center}
    \end{mdframed}
    \vspace{-1em}
    \captionof{figure}{Key pairs and Addresses.}
\end{center}

\MantaPay{} uses two kinds of cryptographic keypairs to build an address, \emph{spending keys}, $\sk$ and $\pk$, and \emph{viewing keys}, $\svk$ and $\pvk$. An address is the pair $(\pk, \pvk)$ of public keys. The keys have the following properties:

\begin{itemize}
    \item Access to a public spending key $\pk$ and public viewing key $\pvk$ represents the ability to send \Asset{s} to the owner of the associated $\sk$.
    \item Access to a secret viewing key $\svk$ represents the ability to reveal shielded \Asset{} information for \Asset{s} belonging to the owner of the associated $\sk$.
    \item Access to a secret spending key $\sk$ represents the ability to spend \Asset{s} that were received under the associated public spending key $\pk$.
\end{itemize}

Participants in \MantaPay{} are represented by their addresses, but they are not unique representations, since one participant may have access to more than one set of secret keys. See \autoref{sec:addresses-and-key-components} for more information on how these keys are constructed and used for spending, viewing, and receiving \Asset{s}.
