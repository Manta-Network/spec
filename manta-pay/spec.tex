...
\begin{definition}[Transfer Configuration]
    ...
    \begin{align*}
        \KDF^{\Trapdoor} &: \KA.\SharedSecret \to \COM.\Trapdoor \\
        \UTXOSet.\Item   &: \UTXO \\
...
\begin{definition}[\Transfer{} Sender]
    ...
    \begin{align*}
        ...
        S.\cm                &:= \COM_{S.\trapdoor}(S.\asset.\ID \,||\, S.\asset.\VALUE) \\
...
\begin{definition}[\Transfer{} Receiver]
    ...
    \begin{align*}
        ...
        R.\asset    &:= \asset \\
        R.\cm       &:= \COM_{R.\trapdoor}(R.\asset.\ID \,||\, R.\asset.\VALUE) \\
        R.\note     &:= \HPKE.\encrypt(R.\rk.\view, R.\esk, R.\asset)
    \end{align*}
\end{definition}

\footnotetext{The ephemeral secret key is not directly chosen by the ledger participants building the \Transfer{}. Instead, it is derived from other \Transfer{} data and the current state of the ledger. See \autoref{def:transfer-validity-statement} for more.}
...
\begin{definition}[\Transfer{} Validity \Statement{}]\label{def:transfer-validity-statement}
    ...
    \begin{enumerate}
        ...
        \item For all $(i, R) \in \Enumerate(T.\receivers)$, the \Receiver{} $R$ is well-formed at index $i$ with respect to \FAIR{}:
            \begin{align*}
                R.\esk      &= \COM^{\EK}_{\tau}(\mathcal{L} \,||\, i \,||\, R.\rk.\spend) \\
                R.\trapdoor &= \KDF^{\Trapdoor}\left(\KA.\agree(R.\esk, R.\rk.\spend)\right) \\
                R.\cm       &= \COM_{R.\trapdoor}(R.\asset.\ID \,||\, R.\asset.\VALUE)
            \end{align*}
            where $\tau \sim \COM^{\EK}.\TrapdoorDistribution$ is a randomly chosen trapdoor and $\mathcal{L}$ is the ledger checkpoint, a constant determined by the current state of the ledger.
    \end{enumerate}
...
\begin{definition}[\Ledger{}-side \Transfer{} Validity]
    ...
    \begin{itemize}
        \item \textbf{Valid Checkpoint}: The checkpoint $P.\mathcal{L}$ is valid and matches the checkpoint of the current ledger state.
