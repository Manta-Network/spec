...
\begin{definition}[Transfer Configuration]
    ...
    \TODO{Add the fact that we have a conversion from $\CRH.\Output$ to $\KA.\SecretKey$, notably for diversifiers.}
\end{definition}
...
\begin{definition}[\Ledger{} \Transfer{} Update]
    ...
    \begin{itemize}
        \item \textbf{Public Updates}: All the relevant public accounts on the \PublicLedger{} are updated to reflect their new balances.
...
\TODO{add semantic transactions}
...
\begin{itemize}
    \item \TODO{commitment scheme: Pedersen}
    \item \TODO{hash function: Pedersen}
    \item \TODO{symmetric-key encryption scheme: AES-GCM with magic-number nonce and no associated data}
    \item \TODO{key-agreement scheme: x25519 elliptic curve Diffie-Hellman key exchange}
    \item \TODO{key-diversification scheme: extension of key-agreement}
    \item \TODO{key-derivation function: Blake2s with magic-number salt}
    \item \TODO{integrated encryption scheme: combination of above encryption protocols}
    \item \TODO{dynamic cryptographic accumulator: Incremental Merkle Tree with Pedersen hashes}
    \item \TODO{non-interactive zero-knowledge proving system: Groth16 and/or PLONK}
\end{itemize}
