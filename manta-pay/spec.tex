...
\begin{definitiontoc}{Dynamic Cryptographic Accumulator}
    ...
    \begin{align*}
        ...
        \contains   &: \Item \times \StateT \to \Option(\Checkpoint \times \Witness) \\
        \verify     &: \Item \times \Checkpoint \times \Witness \to \Bool
    \end{align*}
    with the following properties:
    \begin{itemize}
        ...
        \item \textbf{Provable Membership}: For any initial state $s : \StateT$ and any list of items $I : \List(\Item)$ we can generate the sequences of states:
            ...
            \[\Some(c, w) := \contains(t, s), \,\,\,\,\, \verify(t, c, w) = \True\]
        \item \TODO{security properties}
    \end{itemize}
...
\begin{definition}[Key Components]
    ...
    If we derive the associated public key to $\view$, i.e. $\KA.\derive(\view)$, we get a public key called the \ReceivingKey{}.
\end{definition}
...
\begin{definition}[Transfer Configuration]
    ...
    \TODO{Add the fact that we have a conversion from $\COM.\Output$ to $\KA.\SecretKey$, notably for ephemeral key, i.e. serialize the output and then deserialize into a secret key (need to match the size)}
\end{definition}
...
\begin{definition}[\Transfer{} Receiver]
    ...
    \begin{align*}
        R.\rk       &:= \rk \\
        R.\esk      &:= \esk \\
        R.\trapdoor &:= \KA.\agree(R.d, R.\rk) \\
        R.\asset    &:= \asset \\
        R.\cm       &:= \COM_{R.\trapdoor}(R.\asset) \\
        R.\note     &:= \HPKE.\encrypt(R.\rk, R.\esk, \asset)
    \end{align*}
\end{definition}
...
\begin{definition}[\Transfer{} Validity \Statement{}]\label{def:transfer-validity-statement}
    ...
    \begin{enumerate}
        ...
        \item For all $(i, R) \in \Enumerate(T.\receivers)$, the \Receiver{} $R$ is well-formed at index $i$ with respect to \FAIR{}:
            \begin{align*}
                R.\esk      &= \COM_{\FAIR}(i \,||\, R.\rk.\view) \\
                R.\trapdoor &= \KA.\agree(R.\esk, R.\rk.\view) \\
                R.\cm       &= \COM_{R.\trapdoor}(R.\asset)
            \end{align*}
