...
\begin{definitiontoc}{Hash Function}
    ...
    We can also ask that a hash function be \emph{binding} or \emph{hiding} as in the above \emph{Commitment Scheme} definition if we partition the $\Input$ space into two parts.
...
\begin{definitiontoc}{Key-Agreement Scheme}
    ...
    \begin{itemize}
        ...
        \item \textbf{Passive Security}: Even if an adversary who eavedrops the network communication, she cannot forge the agreed secret unless she knows how to break Diffie-Hellman Problem.
        \item \textbf{Known-key Security}: Suppose an adversary learned a shared secret from the past session, the adversary does not gain any additional information from combining the past key and public visible data for the purpose of deducting future shared secrets.
        \item \textbf{No Key Control}: The shared secrets are determined by both parties, neither party clone can control the outcome of the shared secret by restricting it to lie in some predetermined small set.
    \end{itemize}
\end{definitiontoc}

\begin{definitiontoc}{Symmetric-Key Encryption Scheme}
    A \emph{one-time symmetric-key encryption scheme} $\SYM$ is defined by the schema:
    \begin{align*}
        \Key &: \Type \\
        \Plaintext &: \Type \\
        \Ciphertext &: \Type \\
...
\begin{definition}[\Transfer{} Sender]
    ...
    \begin{align*}
        ...
        S.\cm                              &:= \COM^{\UTXO}(S.\esk, S.\rk.\spend, S.\asset) \\
        \Some\left(S.\cm_z, S.\cm_w\right) &:= \UTXOSet.\contains(S.\cm, \Ledger.\utxos()) \\
        S.\vn                              &:= \COM^{\VN}(S.\cm, S.\sk.\spend) 
    \end{align*}
\end{definition}
...
\begin{definition}[\Transfer{} Receiver]
    ...
    \begin{align*}
        ...
        R.\esk      &:= \esk \\
        R.\asset    &:= \asset \\
        R.\cm       &:= \COM^{\UTXO}(R.\esk, R.\rk.\spend, R.\asset) \\
...
\begin{definition}[\Transfer{} Validity \Statement{}]\label{def:transfer-validity-statement}
    ...
    \begin{enumerate}
        ...
        \item For all $S \in T.\senders$, the \Sender{} $S$ is well-formed:
            \begin{align*}
                S.\rk.\spend                              &= \KA.\derive(S.\sk.\spend) \\
                S.\cm                                     &= \COM^{\UTXO}(S.\esk, \left(S.\rk.\spend, S.\asset\right)) \\
                S.\vn                                     &= \COM^{\VN}(S.\sk.\spend, S.\cm) \\
                \UTXOSet.\verify(S.\cm, S.\cm_z, S.\cm_w) &= \True
            \end{align*}
        \item For all $R \in T.\receivers$, the \Receiver{} $R$ is well-formed:
            \begin{align*}
                R.\cm &= \COM^{\UTXO}(R.\esk, \left(R.\rk.\spend, R.\asset\right))
            \end{align*}
    \end{enumerate}
