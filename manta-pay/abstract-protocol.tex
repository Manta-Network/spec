...
We also make use of the following two functions:
\begin{itemize}
    \item $\iszero : T \to \Bool$ which returns $\True$ whenever the input element is equal to the zero element of $T$ (this function is only defined whenever $T$ has a well-defined zero element).
    \item $\select : \Bool \times T \times T \to T$ which selects a value from a pair, using the left element if the input boolean is $\False$ and the right element if the input boolean is $\True$.
\end{itemize}
