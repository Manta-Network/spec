...
\begin{definitiontoc}{Signature Scheme}
    ...
    \begin{itemize}
        ...
        \item \textbf{Existencial unforgeability against CMA}: Fix $k : \SecretKey$, and denote $pk = \derive(k)$. An adversary who knows the public key $pk$ and can make any number of queries $m_i: \Message$, obtaining valid signatures $\sigma_i \sim \sign(k, m_i)$ cannot forge a pair $(m, \sigma) \not= (m_i, \sigma_i)$ for all $i$ such that $\verify(pk, m, \sigma) = \True$.
    \end{itemize}
\end{definitiontoc}
...
\begin{definitiontoc}{Duplex Sponge}
    A \emph{duplex sponge} $\DS$ is defined by the schema:
    \begin{align*}
        \Item       &: \Type \\
        \StateT     &: \Type \\
        \absorb     &: \List\langle\Item\rangle \times \StateT \to \StateT \\
        \squeeze    &: \mathbb{Z} \times \StateT \to \List\langle\Item\rangle
    \end{align*}
\end{definitiontoc}
...
\begin{definition}[Transfer Configuration]
    ...
    with the following notational conventions:
    \begin{align*}
        \SUTXO        &:= \COM^{\UTXO}.\Output \\
        \UTXO         &:= \VersionTag \times \Bool \times \PublicAsset \times \SUTXO \\
