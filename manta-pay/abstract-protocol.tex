...
\begin{definition}\label{defn:group}
...
\begin{definition}[Spending Key]
    Given a key-agreement scheme $\KA$ with randomized key-derivation function $\rKDF$ we have:
    \[\SpendingKey := \KA.\SecretKey\]

    which can be randomized by $\rKDF.\Randomness$.
\end{definition}

\begin{definition}[Proof Authorizing Key]
    Given a key-agreement scheme $\KA$ with randomized key-derivation function $\rKDF$ and with a signature scheme $\SIG$ we have:
