...
\end{itemize}
First, we randomly sample the simulation trapdoor $\tau = (\alpha, \beta, \delta, x) \leftarrow \mathbb{F}_r^*$. Second, we compute:
\begin{align*}
...
\end{align*} 
where $[y]_1 = y \cdot g$ and $[y]_2 = y \cdot h$ for all $y \in \mathbb{F}_r$. Then we return $\sigma = (\sigma_1, \sigma_2)$. The goal of the trusted setup is to provide the provers and verifiers with $\sigma$ without anyone knowing $\tau$.
\subsection{Phase 1}
